    ...
    def process_data(self):
        while self.robot.step(self.timestep) != -1:
            if self.receiver.getQueueLength() > 0:
                ...
                front_dist, rear_dist, angle, bearing, pos_x, pos_z = struct.unpack(
                    "6f", data)
    ...
    def calculate_route(self, arena_map, start, goal):
        ...
        while not to_explore.empty():
            ...
            for direction, (x, z, dist) in enumerate(walks):
                ...
                if 0 <= cur_x + x < 240 and 0 <= cur_z + z < 240 and arena_map[cur_x + x, cur_z + z] != 1:
                    if distances[cur_x + x, cur_z + z] == -1 or distances[cur_x, cur_z] + dist < distances[cur_x + x, cur_z + z]:
                        ...
                        manhattan_dist = (
                            (cur_x + x - goal[0])**2 + (cur_z + z - goal[1])**2)**0.5
                        distances[cur_x + x, cur_z +
                                  z] = distances[cur_x, cur_z] + dist
                        to_explore.put(
                            (distances[cur_x + x, cur_z + z] + manhattan_dist, (cur_x + x, cur_z + z)))
        else:
            return []
