...
class MobileRobot:
    def __init__(self):
        ...
        self.emitter = self.robot.getDevice("emitter")    # channel 1 robot -> controller
        self.receiver = self.robot.getDevice("receiver")  # channel 2 controller -> robot
    ...
    def distance_data(self, angle, bearing, position):
        ...
        if math.isnan(front_dist): front_dist = float('inf')
        if math.isnan(rear_dist): rear_dist = float('inf')
        if math.isnan(angle): angle = float('inf')
        if math.isnan(bearing): bearing = float('inf')
        if math.isnan(pos_x): pos_x = float('inf')
        if math.isnan(pos_z): pos_z = float('inf')        
        data = struct.pack("6f", front_dist, rear_dist, angle, bearing, pos_x, pos_z)
        ...
        while self.robot.step(self.timestep) != -1:
            if self.receiver.getQueueLength() > 0:
                ...
                if num_waypoints == -1:
                    ...
                    waypoints = struct.unpack('{}f'.format(int(num_waypoints)), data)
